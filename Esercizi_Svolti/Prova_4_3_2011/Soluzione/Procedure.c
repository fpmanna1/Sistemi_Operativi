...
extern int id_c,id_s; // coda client e coda server
...
void client(){
	...
	for(k=0;k<RICHIESTE;k++){
		...
		ret = msgrcv(id_s, &BS, sizeof(Buffer_S)-sizeof(long), getpid(), 0); // receive selettiva in base al pid del client
...
void* Prodotto(void* v){
	...
	pthread_mutex_lock(&mutex);   // gestione in mutua esclusione della coda di risposta
